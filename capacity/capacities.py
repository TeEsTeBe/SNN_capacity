...
class capacity_iterator():
    ...
    def collect(self,inputs,estates):
        ...
        while donext:
            ...
            score=cov_capacity(estates[self.delay+self.window-2:,:], outputs[self.delay+self.window-2:,:], R_inv=self.corrmat)
        ...
        if self.verbose>=-1:
            tag='Total capacity=%(score).3f (%(perc).2f percent)'%{'score':self.totalscore(),'perc':100.0*self.totalscore()/estates.shape[1]}
            print(tag)
...
#            self.current_score=0.0
    ...
    def next(self):
        ...
        if self.delay<=self.delskip or not(self.use_scores) or not(self.monotonous_delay) or (self.use_scores and self.monotonous_delay and (self.current_score>0.0 or (self.delay>self.delskip and self.current_score<=0.0 and self.holding<self.delhold))):
        ...
        if self.window<=self.windowskip or not(self.use_scores) or not(self.monotonous_window) or (self.use_scores and self.monotonous_window and (self.windowpos_score>0.0)): 
            if (self.window<self.maxwindow) and ((self.window<self.maxdel-self.delay+1) or not(self.monotonous_window)):
                if self.variables>1:
                    self.window+=1
        ...
        if not(self.use_scores) or not(self.monotonous_powerlist) or (self.use_scores and self.monotonous_powerlist and (self.window_score>0.0)): 
        ...
        if not(self.use_scores) or not(self.monotonous_variables) or (self.use_scores and self.monotonous_variables and ((self.powerlist_score>0.0) or self.variables==1)): 
        ...
        if self.verbose>=0:
            print('Total score for degree ',str(self.degree),' = ',str(self.variables_score),'(cumulative total = ',str(self.cumscore),')')
        if not(self.use_scores) or not(self.monotonous_degree) or (self.use_scores and self.monotonous_degree and ((self.variables_score>=self.d_threshold)or(self.prev_var_score>=self.d_threshold))): 
