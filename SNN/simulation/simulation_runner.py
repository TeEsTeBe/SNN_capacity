...
class SimulationRunner:
    implemented_input_types = ['uniform_DC_classification', 'uniform_DC_XORXOR', 'uniform_DC_XOR', 'uniform_DC',
                               'uniform_rate', 'step_rate', 'step_DC', 'spatial_rate', 'spatial_DC', 'None',
    ...
    def __init__(self, group_name, run_title, network_type, input_type, step_duration, num_steps, input_min_value,
                 ...
                 background_rate, background_weight, noise_loop_duration, paramfile, data_dir, dt,
                 spike_recorder_duration,
                 raster_plot_duration, batch_steps, add_ac_current, enable_spike_filtering=False,
                 stop_if_statemat_exists=True, num_threads=1):
        logging.basicConfig(filename=f'{run_title}.log', level=logging.DEBUG,
                            format=f'{run_title} %(asctime)s %(levelname)s: %(message)s')
        ...
        self.network.add_spiking_noise(rate=self.background_rate, weight=self.background_weight,
                                       loop_duration=self.noise_loop_duration)
        ...
        self.spike_recorder = state_utils.create_spike_recorder(self.network.populations.values(),
                                                                stop=self.spike_recorder_duration)
    ...
    def _set_input_to_generators(self, start_step, stop_step):
        ...
        ----------
        start_step: int
            index of first step of the input
        stop_step
            index of last step of the input
        ...
        if 'step_' in self.input_type or 'uniform_' in self.input_type:
            if 'XOR' in self.input_type and not 'temporal' in self.input_type:
                ...
                input_binary_strings = [f'{2 ** classes.index(iv):b}'.rjust(len(classes), '0') for iv in
                                        self.input_signal[start_step:stop_step]]
    ...
    def _setup_input_generators(self):
        """ Creates the input generators defined by the input parameters

        Returns
        -------
        input_generators
            nest generator objects
        ...
        if '_rate' in self.input_type and self.network_type == 'microcircuit':
            ...
            input_weight = connection_utils.calc_synaptic_weight(input_weight, scaling_factor_s1, 'exc',
                                                                 self.network.neuron_params_exc['g_L'])
    ...
    def _create_plots(self):
        ...
        rasterplot_spikelist = general_utils.spikelist_from_recorder(self.spike_recorder,
                                                                     stop=self.raster_plot_duration)
    ...
    def run(self):
        """ Runs the simulation, saves the results and creates figures """
        ...
        while start_step < self.num_steps:
            ...
            self.logger.info(
                f'Simulated real time: {simulated_real_time} sec ({round(simulated_real_time / 60, 2)} min, {round(simulated_real_time / 3600, 4)} h)')
            time_still_needed = simulated_real_time / (percentage_done / 100.) - simulated_real_time
            self.logger.info(
                f'\tApproximate simulation time until finished: {time_still_needed} sec ({round(time_still_needed / 60, 2)} min, {round(time_still_needed / 3600, 4)} h)')

            batch_duration = batch_end_real_time - batch_start_real_time
            self.logger.info(
                f'\tBatch took {batch_duration} sec ({round(batch_duration / 60, 2)} min, {round(batch_duration / 3600, 4)}h)\n\n')
