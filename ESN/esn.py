...
class ESN:
    def __init__(self,I2R = np.array([]), B2R = np.array([]), R2R = np.array([]), O2R = np.array([]), nonlinearity = lambda x: np.tanh(x), statenoise = 0.0, inputnoise = 0.0, readoutnoise = 0.0):
        # number of neurons
        self.NN = R2R.shape[0]
        # number of inputs
        self.NI = I2R.shape[1]
    ...
    def Copy(self):
        NewRes = ESN(I2R = self.I2R.copy(), B2R = self.B2R.copy(), R2R = self.R2R.copy(), O2R = self.O2R.copy(), nonlinearity = self.NLfun, statenoise = self.statenoise, inputnoise = self.inputnoise, readoutnoise = self.readoutnoise)
        return NewRes
    ...
    def _Update(self,inputs):
        self.state = self.NLfun(np.dot(inputs,self.I2R) + np.dot(self.state,self.R2R) + self.B2R + self.statenoise*np.random.randn(1,self.NN))

    def _UpdateWithFeedback(self,inputs,feedback):
            self.state = self.NLfun(np.dot(inputs,self.I2R) + np.dot(self.state,self.R2R) + np.dot(feedback,self.O2R) + self.B2R + self.statenoise*np.random.randn(1,self.NN))
